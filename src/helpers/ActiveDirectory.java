...
public class ActiveDirectory {
    ...
    public static User[] getUsers(LdapContext context) throws NamingException {
        ...
        if (authenticatedUser.contains("@")){
            ...
            NamingEnumeration answer = context.search( "OU=Autisti,OU=OU_Utenti,DC=mom,DC=local", "(objectClass=user)", controls);
            try{
                while(answer.hasMore()) {
                    Attributes attr = ((SearchResult) answer.next()).getAttributes();

                    if(attr.get("memberof")!= null && (attr.get("memberof").toString().contains("Autisti_") &&
                            !attr.get("memberof").toString().contains("Autisti_Utenti") &&
                            !attr.get("memberof").toString().contains("Autisti_Autorizzati") &&
                            !attr.get("memberof").toString().contains("Autisti_Scolastici")))
                    {
                        Attribute user = attr.get("sAMAccountName");
                        if (user != null) {
                            if(attr.get("mail") == null){
                                attr.put("mail", "void@void.com");
                            }
    ...
    public static class User {
        ...
        public User(Attributes attr) throws javax.naming.NamingException {
            ...
            userPrincipal = (String) attr.get("sAMAccountName").get();
            if (attr.get("mail") != null) {
